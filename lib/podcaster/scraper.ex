defmodule Podcaster.Scraper do
  ...
  def scrape_series(series_guid) do
    params = [
      ...
      ItemsPerPage: 100,  # Adjust as needed to get all teachings in one request
      PageNumber: 1
    ]
    ...
    case @http_client.get(url) do
      {:ok, %{status_code: 200, body: body}} ->
        with {:ok, document} <- Floki.parse_document(body),
             script when not is_nil(script) <- Floki.find(document, "script[type='application/ld+json']") |> List.first(),
  ...
  defp parse_series([series_data | _], teachings) do
    %Series{
      ...
      image_url: "#{@base_url}/sites/default/files/podcast_image.jpg",  # Update if there's a series-specific image
      teachings: teachings
    }
  end
  ...
  defp parse_teaching(api_data, json_data) do
    ...
    %Teaching{
      ...
      url: @base_url <> api_data["CanonicalUrl"],
